...
let wid = function Widget w -> w.id | Flex _ -> "flex" | Visibility _ -> "visibility"
...
let rec update_ui mouse keyboard widget_cache model tree =
  ...
  | Visibility { id; getter; child; _ } ->
      ...
      Hashtbl.replace widget_cache id (Visible is_visible);
      update_ui mouse keyboard widget_cache model child
...
let layout_ui ?(print_ui_tree = false) scr_width scr_height tree =
  ...
  let get_width = function
    | Widget w -> w.computed_size.width
