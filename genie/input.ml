...
type key_input = { keys : (key * key_transition) list }
...
let mouse_btn_state_transition prev cur =
  match (prev, cur) with
  | true, true -> Held
  | true, false -> Released
  | false, true -> Clicked
...
let get_mouse_input () : mouse_input =
  ...
  let prev_left = Djinn.get_prev_left_btn () in
  let left = Djinn.get_left_btn () in
